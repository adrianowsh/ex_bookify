defmodule ExBookify.Vacation.Booking do
  use Ecto.Schema
  ...
  schema "bookings" do
    ...
    belongs_to :place, ExBookify.Vacation.Place
    belongs_to :user, ExBookify.Accounts.User
  ...
  defp dates_available?(start_date, end_date, place_id) do
    query =
      from booking in ExBookify.Vacation.Booking,
        where:
    ...
    case ExBookify.Repo.all(query) do
      [] -> true
      _ -> false
    end
  end
  ...
  defp calculate_total_price(changeset) do
    case changeset.valid? do
      true ->
        ...
        place = ExBookify.Repo.get!(ExBookify.Vacation.Place, place_id)
